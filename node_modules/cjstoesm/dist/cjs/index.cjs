...
const KNOWN_EXTENSIONS = [".d.ts", ".d.dts.map", ".js.map", ".ts", ".tsx", ".js", ".jsx", ".mjs", ".mjs.map", ".mjsx", ".cjs", ".cjs.map", ".csjx", ".d.cts", ".d.cts.map", ".d.mts", ".d.mts.map", ".json", ".tsbuildinfo"];
/**
...
function getFolderClosestToRoot(root, files) {
    ...
    for (const file of files) {
        const relativeToRoot = path__default.default.relative(root, file);
        if (relativeToRoot.split("/").length < candidate.split("/").length) {
            candidate = relativeToRoot;
        }
    }
    return path__default.default.join(root, path__default.default.dirname(candidate));
}
function normalizeGlob(glob) {
    return path__default.default.extname(glob) === "" && !glob.endsWith("*") ? `${glob}/*` : glob;
}
...
function resolvePath({ id, parent, cwd, prioritizedPackageKeys = ["exports", "es2015", "esm2015", "module", "jsnext:main", "main", "browser"], prioritizedExtensions = ["", ".js", ".mjs", ".cjs", ".jsx", ".ts", ".mts", ".cts", ".tsx", ".json"], moduleDirectory = "node_modules", fileSystem, resolveCache }) {
    ...
    try {
        const resolveResult = path__default.default.normalize(resolve__default.default.sync(id, {
...
const realFileSystem = {
    ...
    mkdirSync: fs__default.default.mkdirSync,
    writeFileSync: fs__default.default.writeFileSync
};
...
class Logger {
    ...
    verbose(...messages) {
        if (this.logLevel < 2 /* LogLevelKind.VERBOSE */)
            return;
        console.log(color__default.default[this.VERBOSE_COLOR]("[VERBOSE]"), ...messages);
    }
    /**
    ...
    debug(...messages) {
        if (this.logLevel < 3 /* LogLevelKind.DEBUG */)
            return;
        console.log(color__default.default[this.DEBUG_COLOR]("[DEBUG]"), ...messages);
    }
    /**
     * Logs warning-related messages
     */
    warn(...messages) {
        console.log(color__default.default[this.WARNING_COLOR](`(!)`), ...messages);
    }
}

function createTaskOptions({ typescript = ts__default.default, fileSystem = realReadonlyFileSystem, debug = false, cwd = process.cwd(), preserveModuleSpecifiers = "external", importAssertions = true, logger = new Logger(debug !== false ? 3 /* LogLevelKind.DEBUG */ : 0 /* LogLevelKind.NONE */) } = {}) {
    return {
        typescript,
        fileSystem,
        debug,
...
async function transformTask(options) {
    ...
    logger.debug("Options:", util.inspect({ input, outDir, cwd, write, debug, preserveModuleSpecifiers, importAssertions }, {
        colors: true,
        depth: Infinity,
        maxArrayLength: Infinity
    }));
    // Match files based on the glob(s)
    const matchedFiles = new Set(ensureArray(input).flatMap(glob => fastGlob__default.default.sync(normalizeGlob(path__default.default.normalize(glob)), { fs: fileSystem }).map(file => (path__default.default.isAbsolute(file) ? path__default.default.normalize(file) : path__default.default.join(cwd, file)))));
    ...
    if (outDir == null) {
        outDir = path__default.default.join(closestFolderToRoot, TEMPORARY_SUBFOLDER_NAME);
    }
    ...
    program.emit(undefined, (fileName, text) => {
        const newFilename = path__default.default.normalize(fileName).replace(`/${TEMPORARY_SUBFOLDER_NAME}`, ``);
        const nativeNormalizedFileName = path__default.default.native.normalize(newFilename);
        ...
        logger.info(`${color__default.default.green("✔")} ${path__default.default.native.relative(cwd, nativeNormalizedFileName)}`);
...
function createTransformTaskOptions({ fileSystem = realFileSystem, write = true, input, hooks = {}, outDir, ...rest }) {
    ...
    return {
        ...
        outDir: outDir == null ? undefined : path__default.default.normalize(path__default.default.isAbsolute(outDir) ? outDir : path__default.default.join(taskOptions.cwd, outDir))
    };
}
