			...
			case 'blacklist':
				switch (interaction.options.getSubcommand()) {
					case 'add': {
						...
						await db.set('blacklist', [...blacklist, user.id]);
					...
					case 'rm': {
						{
							...
							await db.set(
								'blacklist',
								blacklist.filter((id: Snowflake) => id !== user.id)
							);
							await interaction.reply('Done.');
							break;
						}
			...
			case 'whitelist': {
				switch (interaction.options.getSubcommand()) {
					case 'add': {
						...
						await db.set('whitelist', [...whitelist, user.id]);
						await interaction.reply('Done.');
						break;
					}
					case 'ls': {
					...
					case 'rm': {
						...
						await db.set(
							'whitelist',
							whitelist.filter((id: Snowflake) => id !== user.id)
						);
						await interaction.reply('Done.');
						break;
					}
