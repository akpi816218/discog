...
('use strict');
const db = new Jsoning('coin.db.json');
...
export const execute = async (interaction) => {
    switch (interaction.options.getSubcommand(false)) {
        case 'mine':
            let mcoins;
            if (!db.get(interaction.user.id.toString()))
                mcoins = 0;
            else
                mcoins = db.get(interaction.user.id.toString()).coins;
            ...
            await interaction.reply(`${userMention(interaction.user.id)} dug up ${add} coins and now has ${mcoins}.`);
            break;
        case 'gamble':
            await interaction.reply('This feature is still in development! Check back soon!');
            break;
        case 'show':
            ...
            if (!user) {
                ...
                if (ubcoins == null)
                    ubcoins = 0;
                await interaction.reply(`${userMention(user.id)} has ${ubcoins} coins.`);
            }
            break;
        case 'leaderboard':
            let all = db.all();
            let arr = [];
            for (let prop in all)
                arr.push([prop, all[prop]]);
            arr.sort((a, b) => b[1].coins - a[1].coins);
            let embed = new EmbedBuilder()
                .setTitle('DisCog Currency Leaderboard')
                .setDescription(`This leaderboard might be outdated. Run ${inlineCode('/coin leaderboard')} to get a new one.`)
                .setTimestamp();
            let fields = [];
            arr.forEach((val) => {
                fields.push({
