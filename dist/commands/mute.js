...
export const execute = async (interaction) => {
    let user = await interaction.member.fetch(true), member = await interaction.guild.members.fetch(interaction.options.getUser('user').id);
    ...
    if (!member.manageable) {
        ...
        let guildObj = db.get(member.guild.id), roles = guildObj[member.id];
            ...
            await interaction.reply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .addFields({ name: 'Acting User:', value: userMention(user.id) }, { name: 'Unmuted User:', value: userMention(member.id) })
                        .setFooter({
        ...
        await interaction.reply({
            embeds: [
                new EmbedBuilder()
                    .setTitle('User Muted')
                    .addFields({ name: 'Acting User:', value: userMention(user.id) }, { name: 'Muted User:', value: userMention(member.id) })
                    .setFooter({
                    text: 'Powered by DisCog',
                    iconURL: interaction.client.user.displayAvatarURL(),
                }),
