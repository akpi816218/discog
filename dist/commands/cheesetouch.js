...
export const execute = async (interaction) => {
    ...
    if (!interaction.options.getBoolean('force', false)) {
        if (interaction.user.id == db.get('cheesetouch')) {
            ...
            await interaction.reply(`${userMention(interaction.user.id)} gave the cheese touch to ${userMention(user.id)}.`);
        }
        else {
            await interaction.reply(`${userMention(db.get('cheesetouch'))} has the cheese touch, not you.`);
        }
    }
    else if ([
        ...
        await db.set('cheesetouch', user.id);
        await interaction.reply(`${userMention(interaction.user.id)} ${bold('forcefully')} gave the cheese touch to ${userMention(user.id)}.`);
    }
    else {
        await interaction.reply('You do not have the permission to use this command in this way.');
    }
};
export default {
    data,
